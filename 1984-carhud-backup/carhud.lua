...
local cruisekey = 137 -- F9
local seatbeltkey = 21 -- K
...
Citizen.CreateThread(function()
	...
	--  ####         	 PLANE HUD	  	  	  ####  --			
			...
			if HUD.SeatBelt and PedBike == false and PedHeli == false and PedPlane == false and PedBoat == false then
				...
				if car ~= 0 and (wasInCar or IsCar(car)) then
					...
					speedBuffer[1] = GetEntitySpeed(car)
						...
						if beltOn then
							drawTxt(UI.x + 0.514, UI.y + 1.245, 1.0,1.0,0.45, "SEATBELT", 0, 255, 0,200) -- Green
						else
							drawTxt(UI.x + 0.514, UI.y + 1.245, 1.0,1.0,0.45, "SEATBELT", 255, 0, 0,200) -- Red
						end
			...
			if HUD.Cruise and PedHeli == false and PedPlane == false and PedBoat == false then
				...
				if IsControlJustPressed(1, cruisekey) and cruisecolor == false then -- F9
					cruisecolor = true
